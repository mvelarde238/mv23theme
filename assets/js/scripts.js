...
window['Adaptive_Navbars'] = function () {
  ...
  Adaptive_Navbar.prototype = {
    _doAdapt: function _doAdapt() {
      ...
      var stopWidth = moreLi.offsetWidth;
      var hiddenItems = [];
    ...
    _outsideclick: function _outsideclick() {
  ...
  Adaptive_Navbar.create = function (el) {
    var instance = new Adaptive_Navbar(el);
...
function adjustScrollPosition(anchor) {
  ...
  if (targetElement) {
    ...
    var headerHeight = parseInt(sticky_header_height);
...
(function ($, c) {
  $(function () {
    ...
    function set_header_height() {
      var timeoout = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 800;
      setTimeout(function () {
        // let css transitions change elements height
        var header = document.getElementById("header"),
...
window['OffCanvas_Elements'] = function () {
  ...
  Offcanvas_Element.prototype = {
    ...
    _handle_styles: function _handle_styles() {
      var settings = this.settings,
        ...
        type = this.type;
      var modal_content = offcanvas_element.querySelector('.modal-content');
      ...
      if (settings.hasOwnProperty('padding') && settings.padding.use) {
        for (var _i82 = 0, _Object$entries8 = Object.entries(settings.padding); _i82 < _Object$entries8.length; _i82++) {
          var _Object$entries8$_i = _slicedToArray(_Object$entries8[_i82], 2),
            key = _Object$entries8$_i[0],
            value = _Object$entries8$_i[1];
          if (['top', 'right', 'bottom', 'left'].includes(key)) {
            var capitalized_key = key.charAt(0).toUpperCase() + key.slice(1);
            var propertyName = 'padding' + capitalized_key;
    ...
    _handle_close_on_click_setting: function _handle_close_on_click_setting() {
      var settings = this.settings,
        offcanvas_element = this.offcanvas_element,
        type = this.type;
      console.log();
      var close_on_click = settings.close_on_click || false;
      if (close_on_click) {
        var close_on_click_class = type === 'sidenav' ? 'sidenav-close' : 'modal-close';
        offcanvas_element.querySelectorAll('a').forEach(function (element) {
  ...
  Offcanvas_Element.create = function (element_data) {
    ...
    if (_typeof(instance.M_instance) === "object") {
  ...
  Offcanvas_Element.getElementById = function (id) {
    var _instance = null;
    id && Offcanvas_Element.getInstances().forEach(function (_ins) {
          ...
          if (carrusels.length) {
            for (var i = 0; i < carrusels.length; i++) {
              ...
              if (carrusel) {
                ...
                for (var _i83 = 1; _i83 <= nth_slides; _i83++) {
                  if (e.progress >= 1 / nth_slides * (_i83 - 1) && e.progress <= 1 / nth_slides * _i83) {
                    var slide_should_be_here = _i83 - 1; // slide index starts in 0
...
window['Theme_Navbars'] = function () {
  ...
  Theme_Navbar.prototype = {
    _handle_toggle_submenu_button: function _handle_toggle_submenu_button() {
      ...
      var _loop2 = function _loop2() {
        ...
        var toggle_submenu_button = item_has_children.querySelector(':scope > .toggle-submenu');
        toggle_submenu_button.addEventListener('click', function (event) {
    ...
    _show_submenu: function _show_submenu(item_has_children) {
      var rect = item_has_children.getBoundingClientRect();
      var outOfBoundsOnX = document.body.clientWidth - (rect.x + sub_menu_width) < sub_menu_width;
      var openFromClass = outOfBoundsOnX ? 'open-from-right' : 'open-from-left';
      var sub_menu = item_has_children.querySelector(':scope > .sub-menu');
      sub_menu.classList.add(openFromClass);
      sub_menu.style.display = 'block';
    },
    _hide_submenu: function _hide_submenu(item_has_children) {
      var sub_menus = item_has_children.querySelectorAll('.sub-menu');
      ...
      var toggle_submenu_buttons = item_has_children.querySelectorAll('.toggle-submenu');
      for (var _i84 = 0; _i84 < sub_menus.length; _i84++) {
        var button = toggle_submenu_buttons[_i84];
        button.setAttribute('aria-expanded', false);
      }
    },
    _handle_mouse_event: function _handle_mouse_event(event, item_has_children, callback) {
  ...
  Theme_Navbar.create = function (el) {
    var instance = new Theme_Navbar(el);
