        ...
        $(elem).fadeOut(0).waypoint(function (direction) {
          if (direction === 'down') {
            var animation = this.element.dataset.animation,
                ...
                options = {}; // options.delay = (delay) ? parseInt(delay) : 0;
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (mapas.length) {
      var initMap = function initMap(element) {
        ...
        var marker = new google.maps.Marker(marker_options); // map.setOptions({styles: styleArray});
      };
  ...
  $(function () {
    ...
    function colorbox_group() {
      $(".zoom").colorbox({
        ...
        maxHeight: "96%",
        maxWidth: "96%",
        onComplete: function onComplete() {// $('#cboxLoadedContent').zoom({ on:'click' });
        }
      });
    }
