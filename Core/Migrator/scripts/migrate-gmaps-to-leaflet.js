jQuery(document).ready(function($) {
    ...
    function processNextBatch() {
        $.ajax({
            ...
            method: 'POST',
            data: {
                action: 'process_gmaps_to_leaflet',
    ...
    // Init the process
    $(document).on('click', '.theme-migrator__init-gm-to-leaflet[data-status="initial"]', function(){
        $migration_btn = $(this);
        $migration_btn.attr('data-status','processing');
        console.log('Migrating maps components data...');
        processNextBatch();
    });
});
