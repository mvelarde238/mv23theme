jQuery(document).ready(function($) {
    ...
    function processNextBatch() {
        $.ajax({
            ...
            method: 'POST',
            data: {
                action: 'process_new_oce_settings',
    ...
    // Init the process
    $(document).on('click', '.theme-migrator__init-new-oce-settings[data-status="initial"]', function(){
        $migration_btn = $(this);
        $migration_btn.attr('data-status','processing');
        console.log('Migrating Off-Canvas Elements Settings data...');
        processNextBatch();
    });
});
