jQuery(document).ready(function($) {
    ...
    function processNextBatch() {
        $.ajax({
            ...
            method: 'POST',
            data: {
                action: 'process_scrollmagic_to_gsap',
    ...
    function afterDataMigration() {
        ...
        $.ajax({
            ...
            method: 'POST',
            data: {
                action: 'after_scrollmagic_to_gsap',
                nonce: THEME_MIGRATOR_GLOBALS.nonce
            },
            success: function(response) {
                if (response.success) {
    ...
    // Init the process
    $(document).on('click', '.theme-migrator__init-sm-to-gsap[data-status="initial"]', function(){
        $migration_btn = $(this);
        $migration_btn.attr('data-status','processing');
        console.log('Migrating scroll-animations settings data...');
        processNextBatch();
    });
});
