jQuery(document).ready(function($) {
    ...
    function processNextBatch() {
        $.ajax({
            ...
            method: 'POST',
            data: {
                action: 'process_tm_group_to_groups',
    ...
    function afterDataMigration() {
        ...
        $.ajax({
            ...
            method: 'POST',
            data: {
                action: 'after_tm_group_to_groups',
                nonce: THEME_MIGRATOR_GLOBALS.nonce
            },
            success: function(response) {
                if (response.success) {
    ...
    // Init the process
    $(document).on('click', '.theme-migrator__init-tm-group-to-groups[data-status="initial"]', function(){
        $migration_btn = $(this);
        $migration_btn.attr('data-status','processing');
        console.log('Migrating Timeline group to groups data...');
        processNextBatch();
    });
});
