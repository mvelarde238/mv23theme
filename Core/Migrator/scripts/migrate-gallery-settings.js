jQuery(document).ready(function($) {
    ...
    function processNextBatch() {
        $.ajax({
            ...
            method: 'POST',
            data: {
                action: 'process_new_gallery_settings',
    ...
    function afterDataMigration() {
        ...
        $.ajax({
            ...
            method: 'POST',
            data: {
                action: 'after_new_gallery_settings',
                nonce: THEME_MIGRATOR_GLOBALS.nonce
            },
            success: function(response) {
                if (response.success) {
    ...
    // Init the process
    $(document).on('click', '.theme-migrator__init-new-gallery-settings[data-status="initial"]', function(){
        $migration_btn = $(this);
        $migration_btn.attr('data-status','processing');
        console.log('Migrating Gallery Settings data...');
        processNextBatch();
    });
});
