...
class Migrate_1_5_X_to_2_0_1{
    ...
    public static function getInstance() {
        if (self::$instance == null) {
            self::$instance = new Migrate_1_5_X_to_2_0_1();
        }
    ...
    public function migrate(){
        ...
        add_action( 'wp_ajax_process_page_data_to_2_0_1', array($this, 'ajax_process_page_data') );
        add_action( 'wp_ajax_after_data_migration_to_2_0_1', array($this, 'ajax_after_data_migration') );
    }
    ...
    public function display(){
        ...
        <div class="wrap">
            <div class="theme-migrator">
                <h3>Migrate 1.5.X to 2.0.1 - Columns Layout Field replace Components Selector</h3>
                <button class="theme-migrator__init-2-0-1 button-primary" data-status="initial">
    ...
    public function migrate_component_data( $component ){
        ...
        // $new_component['__lel'] = 'lel';
        
        if( $component['__type'] == 'columns' || $component['__type'] == 'inner_columns' ){

            $new_component['__type'] = ($component['__type'] == 'columns') ? 'row' : 'inner_row';
            $new_component['row'] = array(
                'content' => array(),
                'row_settings' => array( '__tab' => 'Desktop' ),
            ...
            for ($i=1; $i <= $quantity; $i++) { 
                $new_component['row']['content'][$count] = $component['column_'.$i];
                $count++; 
            }

            // migrate row settings
            foreach ($devices as $device_key) {
                $old_grid_value = $component[$device_key.'_grid_'.$quantity] ?? '';
                $new_component['row']['row_settings'][$device_key.'_grid_'.$quantity] = $old_grid_value;
                $new_component['row']['row_settings'][$device_key.'_gap'] = $component[$device_key.'_gap'] ?? 20;
            }

            // migrate columns settings
            $count = 0;
            for ($i=1; $i <= $quantity; $i++) { 
                $settings = $component['column_'.$i.'_settings']['settings'];
                if( !is_array( $settings ) ) $settings = array();
                $settings['__tab'] = 'Desktop';

                foreach ($devices as $device_key) {
                    $old_order_name = ($device_key == 'l') ? 'order' : (($device_key == 't') ? 'tablet_order' : 'mobile_order');
                    $old_order = $component['column_'.$i.'_settings'][ $old_order_name ] ?? '';
                    $settings[$device_key.'_order'] = $old_order;

                    $old_alignment_name = ($device_key == 'l') ? 'content_alignment' : (($device_key == 't') ? 'tablet_content_alignment' : 'mobile_content_alignment');
                    $old_alignment = $component['column_'.$i.'_settings'][$old_alignment_name] ?? '';
                    $settings[$device_key.'_content_alignment'] = $old_alignment;
                }

                $new_component['row']['columns_settings'][$count] = $settings;
                $count++; 
            }

            // unset old settings format
            $unset_columns_properties = array( 
                'column_1','column_1_settings',
                'column_2','column_2_settings',
                'column_3','column_3_settings', 
                'column_4','column_4_settings',
                'l_grid_1', 't_grid_1', 'm_grid_1',
                'l_grid_2', 't_grid_2', 'm_grid_2',
                'l_grid_3', 't_grid_3', 'm_grid_3',
                'l_grid_4', 't_grid_4', 'm_grid_4',
                'l_gap', 't_gap', 'm_gap', 
                'components', 'quantity'
            );
            foreach ($unset_columns_properties as $key) {
    ...
    public function has_inner_components( $component_type ){
        $has_inner_components = apply_filters('has_inner_components_filter', array(
            'columns' => array( 'column_1', 'column_2', 'column_3', 'column_4' ),
            // 'inner_columns' => array( 'column_1', 'column_2', 'column_3', 'column_4' )
        ));
