...
- Fix Carousel to read columns and columns_gap from the correct args when used inside related-posts components
- Accessibility Improvements on Modals: added aria-modal, aria-hidden and role attributes to modals for better accessibility and screen reader support, 
  and added focus management to ensure that focus is properly handled when modals are opened and closed
...
- Implemented conditional comparison operators in Handlebars
...
- if the offcanvas element is dismissible, show the close button, if not hide it
...
- custom_global_css option implementation on global styles to allow users to add custom CSS that will be applied globally on the site, 
  with a textarea field in the global styles settings for easy management
- Fix: ensure global styles and theme options datastore changes are properly handled when previewing the page on the builder, 
...
- Insert style tag on post modal content to prevent losing single template styles on postcard preview
...
- Remove grid-stack-gallery margins support (not working in columns layout)
...
- set noUndo option when changing the __wrapperReference on carousel items to prevent unnecessary undo steps
...
- Implement Carousel::showCurrentSlideIndex()
...
- Internationalization: Add missing translation keys for adaptive navbar label
- Fix bug on Gallery shortcode: missing .marquee-track opening tag
...
- Fix component:clone event to consider the case when component use delegate attrribute (flipbox)
...
- Fix: Ensure the custom_datastore_change_callback is properly called on all flip box components to synchronize changes 
  ...
  in the main component's datastore and view
...
- Fix gap command on context menu options to work with the new update-css-property
- Flipbox component review
...
- Update translation files
- Fix menu and testimonials widths on builder
...
- Fix Template_Generator structure
...
- editor.getPreparedFileObject() method, get object from UltimateFields.Field.File.Cache
...
- Fix undefined COLOR_PICKER of uf library
...
- Update language files
- Fix Carousel Comp: fade_color can be null
...
- Migrate Carrusel Comp. to Carousel Component (!use migrator)
...
- Include slider on PAGE_HEADER_COMPONENTS constant
